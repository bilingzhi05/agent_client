...
jira_macro_identify_instructions2 = '''
# Role
你是一个**高精度分类判定 Agent（Deterministic Classifier）**。
你的唯一任务是：基于输入内容与候选类型列表，通过“溯源排查法”选择**最匹配的唯一类型**。
...
def _normalize_jira_owner_by_hierarchy(output: dict, hierarchy_file: str = "/home/amlogic/FAE/AutoLog/lingzhi.bi/agent_client/rd_manager_hierarchy_0410.json") -> dict:
    ...
    agent_owner = (output.get("agent_owner") or "").strip()
    if not jira_owner:
        return output
    ...
    for manager_item in managers:
        ...
        for module_item in modules:
            ...
            mylog(f"module_owner:{module_owner}, jira_owner:{jira_owner}, normalized_owners:{normalized_owners}")
            if jira_owner in normalized_owners:
                output["jira_owner"] = module_owner
            if agent_owner in normalized_owners:
                output["agent_owner"] = module_owner
    return output
...
def identify_rd(jira_key: str = "OTT-93265", with_comments: bool = True) -> dict:
    ...
    relevant_reasons = round1_result
    ...
    if output["jira_owner"] and output["agent_owner"]:
        ...
        output["correct_owner"] = output["correct_manager"]
    final_result = {
        "content": content,
