...
from common.config import (
    ...
    LLM_DEESPEEK_MAX_CHAR_LENGTH,
    VL_OLLAMA_MODEL_CONFIG,
    LLM_DEEPSEEK_CONTEXT_CONTEXT_MAX_CHAR_LENGTH,
...
class SimpleImpAgent(AgentBase):

    def __init__(self, model_args:dict= {}, prompt: [list[str], str] = None, use_think_content: bool = True, with_format_output: bool = True, vl_model: bool = False, output_json: bool = True, tools: list = None, **kwargs):
        #检查提示词是否为空
    ...
    #提取不超过固定字符的内容的函数
    def extract_content(self, content: list[str] = None, start_index: int = 0, max_length: int = LLM_DEESPEEK_MAX_CHAR_LENGTH):
        """
    ...
    def run_with_chunks(self, context: str = "", chunks : list[str] = None, images: list[Image] = None, debug_mode: bool = False):
        ...
        if not chunks:
            context = truncate_content(context, LLM_DEESPEEK_MAX_CHAR_LENGTH)
        else:
            context = truncate_content(context, LLM_DEEPSEEK_CONTEXT_CONTEXT_MAX_CHAR_LENGTH)
        # 提取不超过最大字符长度的内容
