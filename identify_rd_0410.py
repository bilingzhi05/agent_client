...
# Core Principles
1. 分类是严格的“文本匹配”任务，不是推理任务。
2. 只能依据“判定依据”文本中**已经出现的词语**进行匹配。
3. 如果判定依据中没有出现与待分类内容直接匹配的关键词，该类型禁止被选择。
4. reason 必须引用判定依据中的原文片段。
5. reason 中禁止出现不在判定依据文本中的词。
6. 出现“但 / 可能 / 推断 / 推测 / 相关 / 辅助 / 涉及 / 间接”等词视为违规。
7. 不允许构建因果链路，只允许说明“文本命中关系”。
...
def _load_macro_map_data() -> dict:
        map_path = Path(__file__).resolve().parent / "rd_manager_hierarchy_0410.json"
...
def _build_macro_candidates(macro_data: dict) -> list[dict]:
    candidates = []
    for item in macro_data.get("RD Manager", []):
        candidates.append(
            {
                "name": (item.get("name") or "").strip(),
                "basis": (item.get("responsibility") or "").strip(),
            }
        )
...
def _build_owner_candidates(selected_macro: dict) -> list[dict]:
    candidates = []
    for comp in selected_macro.get("modules", []):
        owner = (comp.get("module_owner") or "").strip()
        basis = ("; ".join(comp.get("Issues_classification", [])) or "").strip()
        component = ("; ".join(comp.get("JIRA Component", [])) or "").strip()
        if owner:
            candidates.append(
                {
                    "name": owner,
...
def _normalize_jira_owner_by_hierarchy(output: dict, hierarchy_file: str = "/home/amlogic/FAE/AutoLog/lingzhi.bi/agent_client/rd_manager_hierarchy_0410.json") -> dict:
    jira_owner = (output.get("jira_owner") or "").strip()
    if not jira_owner:
        return output
    ...
    try:
        ...
        return output
    managers = hierarchy_data.get("RD Manager") if isinstance(hierarchy_data, dict) else []
    if not isinstance(managers, list):
        return output
    for manager_item in managers:
        modules = manager_item.get("modules") if isinstance(manager_item, dict) else []
        if not isinstance(modules, list):
            continue
        for module_item in modules:
            if not isinstance(module_item, dict):
                continue
            module_owner = (module_item.get("module_owner") or "").strip()
            default_owners = module_item.get("jira_default_owners") or []
            if not module_owner or not isinstance(default_owners, list):
                continue
            normalized_owners = [str(owner).strip() for owner in default_owners]
            if jira_owner in normalized_owners:
                output["jira_owner"] = module_owner
                return output
    return output
...
def identify_rd(jira_key: str = "OTT-93265", with_comments: bool = True) -> dict:
    ...
    # 找到对应的manager信息
    selected_macro = None
    for item in macro_map_data.get("RD Manager", []):
        if (item.get("name") or "").strip() == macro_type:
            selected_macro = item
            break
    # 获取leader信息         
