...
# def deep_clean_json(s):
#     # 1. 替换不可见字符
#     s = s.replace('\xa0', ' ').replace('\u202c', '')
    
#     # 2. 智能替换作为分隔符的中文引号 (避免误伤字符串内部的引号)
#     s = re.sub(r'([\[{,]\s*)“', r'\1"', s)
#     s = re.sub(r'”(\s*:)', r'"\1', s)
#     s = re.sub(r'(:\s*)“', r'\1"', s)
#     s = re.sub(r'”(\s*[,}\]])', r'"\1', s)
...
#     # 4. 尝试修复未转义的双引号 (如果引号出现在非分隔符位置，可能是内容，尝试转义?)
#     # 这一步比较冒险，暂时不加，依赖 json5 的宽容度
#     return s
def fix_response_string(text: str) -> str:
    key = '"response"'
    idx = text.find(key)
    if idx == -1:
        return text
...
def parse_llm_json(response):
    ...
    # 3. 移除 JSON 内部的非法注释 (包括 // 和 /* */)
    # 使用 (?m)^\s*//.*$ 仅匹配行首注释，避免误伤 URL (http://)
    text = re.sub(r'(?m)^\s*//.*$', '', text)
    text = re.sub(r'/\*.*?\*/', '', text, flags=re.S)
    
    # 4. 字符转换
    text = deep_clean_json(text)
    ...
    try:
        ...
        try:
            # 7. 兜底尝试闭合括号后再解析
            return json5.loads(fix_json_closure(text))
        except Exception as e:
            # 8. 记录错误日志
...
def extract_belong_module_from_analyze_result(text: str) -> str | None:
    """
    从分析文本中提取 【所属模块】 的内容
...
def print_log_with_box(log_content, logger_object=None, log_level="debug"):
    # 内容超长时做省略显示，最多保留 500 字符
    max_show_len = 1000
    show_content = log_content
    if log_content and len(log_content) > max_show_len:
        show_content = log_content[:max_show_len] + " ...（省略后续内容）"
    ...
    if log_content is None or len(f"{log_content}".strip()) == 0:
        ...
        if log_level == "debug":
            logger_object.debug("\n=========================================\n")
            logger_object.debug(show_content)
            logger_object.debug("\n=========================================\n")
        elif log_level == "info":
            logger_object.info("\n=========================================\n")
            logger_object.info(show_content)
            logger_object.info("\n=========================================\n")
        else:
            logger_object.error("\n=========================================\n")
...
def  clean_log_stage_one_multi(src_dir, file_name, dest_dir, duplicate_map_list):
    ...
    try:
        ...
        with open(path, 'r', encoding="utf-8", errors="replace") as file:
            ...
            if player_all_content:
                ...
                for _, index in sorted(player_all_content.items(), key=lambda item: item[1]):
                    if 0 <= index < total_lines:
                        result_line_list.append(f"Line {index}: " + lines[index])
                result_lines = result_line_list
                # result_lines = filter_log_errors(result_line_list, None, None)
                write_to_file_normal(clean_file_path,''.join(result_lines))
            else:
    ...
    return clean_file_path, player_all_content
def  clean_log_str(content: str = ""):
    player_all_content = dict()
    result_line_list = []
    try:
        head_column_pattern = '[0-9-]+\s+[0-9:.]+\s+[0-9]+\s+[0-9]+\s+\w\s'
        ...
        for line in content_lines:
            ...
            # Remove line number prefix first
            modified_line = re.sub(r'(?i)^line\s+\d+:', '', modified_line)
            modified_line = re.sub(head_column_pattern, '', modified_line)
            modified_line = re.sub(number_pattern, '', modified_line)
            modified_line = re.sub(special_symbol, ' ', modified_line)
            modified_line = re.sub(one_symbol, ' ', modified_line)
            modified_line = re.sub(no_special_symbol_without_n, ' ', modified_line)
            modified_line = re.sub(log_tag_level, '', modified_line)
            modified_line = clean_stop_words(modified_line)
            #如果modified_line是空行,则不加入结果
            if not modified_line.strip():
                continue
            modified_line += '\n'
            player_all_content[modified_line] = count
    except Exception as e:
        print("clean_log_str An unexcepted error occured:",e)
        result_line_list.append("查询报错：An unexcepted error occured:" + str(e))

    if player_all_content:
        # 根据 index 从小到大排序并写入结果
        for _, index in sorted(player_all_content.items(), key=lambda item: item[1]):
            if 0 <= index < total_lines:
                result_line_list.append(content_lines[index])
    else:
        print(f'result is empty after clean!')
...
def build_yield_response(event, content):
    return {"event": event, "body": content}
...
def sanitize_filenames(directory_path: str):
    """
    检查指定目录下（递归）的所有文件名和目录名，
    若包含空格或特殊字符，则重命名为下划线。
    """
    result = True
    try:
        if not os.path.isdir(directory_path):
            print(f"[WARN] 路径不存在: {directory_path}")
            result = False
            return result

        # topdown=False 确保先处理子文件/子目录，最后处理父目录，避免重命名导致路径失效
            ...
            for filename in files:
                ...
                if new_name != filename:
                    ...
                    try:
                        ...
                        print(f"[INFO] 重命名文件: {filename} -> {new_name}")
                    except OSError as e:
                        print(f"[ERROR] 重命名文件失败: {old_path} -> {new_path}, {e}")
    except Exception as e:
        print(f"[ERROR] 处理目录 {directory_path} 时发生错误: {e}")
        result = False
    return result
        # 处理目录
        # for dirname in dirs:
        #     new_name = re.sub(r'[\s\(\)\[\]\{\}\'"|&;]+', '_', dirname)
        #     if new_name != dirname:
        #         old_path = os.path.join(root, dirname)
        #         new_path = os.path.join(root, new_name)
        #         try:
        #             os.rename(old_path, new_path)
        #             print(f"[INFO] 重命名目录: {dirname} -> {new_name}")
        #         except OSError as e:
        #             print(f"[ERROR] 重命名目录失败: {old_path} -> {new_path}, {e}")
...
#     pprint(json_data_tmp)
    ...
    "response": "【思考】直接搜索 `compare_pkt` 函数的定义，确认其实现、参数和返回值，以总结其功能。"
}
    '''
